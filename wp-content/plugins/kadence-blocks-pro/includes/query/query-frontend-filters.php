...
class Query_Frontend_Filters {
			...
			if ( 'wordpress' == $source && 'post_author' == $post_field ) {
				...
				if ( is_array( $results ) && $results ) {
					$options_array = array_map( function( $result ) use ( $post_field, $attrs, $show_result_count ) {
						// return array();
						return self::option_format( $result->$post_field, $result->$post_field, $result->count, $show_result_count );
					}, $results );
				}
			}
		}
	...
	public static function update_result_counts( &$options_array, $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $post_type, $post_field, $query_args, $hash ) {
		...
		$secondArrayMap = [];
		foreach ($new_options_array as $item) {
			$secondArrayMap[$item['value']] = $item;
		}

		foreach ($options_array as &$item) {
			if (isset($secondArrayMap[$item['value']])) {
				$item['label'] = $secondArrayMap[$item['value']]['label'];
				$item['count'] = $secondArrayMap[$item['value']]['count'];
			} else {
				$item['count'] = 0;
				$item['label'] = preg_replace('/\(\d+\)/', '(0)', $item['label']);
			}
		}
	}
